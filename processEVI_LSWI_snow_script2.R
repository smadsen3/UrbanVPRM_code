...
#MODIS Snow Cover
mod_snow_files_aqua <- list.files('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover',pattern = 'MYD10A1.006_NDSI_Snow_Cover_doy2018')
mod_snow_files_terra <- list.files('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover',pattern = 'MOD10A1.006_NDSI_Snow_Cover_doy2018')
...
for (i in 1:length(mod_files)){
  ...
  doy<-as.numeric(substr(mod_files[i],33,35))
  sno<-NULL
  if (doy<358){
    for (j in doy:(doy+8)){
      if (length(sno)==0){
        sno <- crop(stack(paste0('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover/',
                                mod_snow_files_aqua[j])),LC_dat)
        sno[values(sno)>100]<-NA
      
        tryCatch({
        if (doy<10){
          snoT <- crop(stack(paste0('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover/',
                                  mod_snow_files_terra[grep(paste0('doy201800',j),mod_snow_files_terra)])),LC_dat)
        }else if (doy < 100){
          snoT <- crop(stack(paste0('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover/',
                                  mod_snow_files_terra[grep(paste0('doy20180',j),mod_snow_files_terra)])),LC_dat)
        }else{
          snoT <- crop(stack(paste0('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover/',
                                  mod_snow_files_terra[grep(paste0('doy2018',j),mod_snow_files_terra)])),LC_dat)
        }
        snoT[values(snoT)>100]<-NA
        sno<-stack(sno,snoT)}, error = function(e){print(paste0('Missing Terra snow data on doy: ', j))})
      } else {
        sn0<-crop(stack(paste0('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_Snow_Cover/',
                             mod_snow_files_aqua[j])),LC_dat)
        sn0[values(sn0)>100]<-NA
        sno<-stack(sno,sn0)
  ...
  mod_sno<-raster::mean(sno,na.rm=TRUE)*0.01
  #mod_sno[which(values(mod_sno)<=1 & values(mod_sno)>=0)]<-1-mod_sno[which(values(mod_sno)<=1 & values(mod_sno)>=0)]
  mod_sno[values(mod_sno)>=0.6]<-0
  mod_sno[values(mod_sno)<0.3]<-1
  mod_sno[which(values(mod_sno)>=0.3 & values(mod_sno)<0.6)]<-0.5
  mod_sno[is.na(mod_sno)]<-1
  ...
  file <- stack(mod_file_red,mod_file_NIR,mod_file_blue,mod_file_SWIR)
  ...
  ### calculate EVI
  file[[5]] <- 2.5 * ((file[[2]] - file[[1]]) / (file[[2]] + 6 * file[[1]] - 7.5 * file[[3]] + 1))*mod_sno
  #file[[5]][abs(file[[5]])>1]<-NA
  ## calculate LSWI
  file[[6]] <- (file[[2]] - file[[4]]) / (file[[2]] + file[[4]])
  #Replace eroneous values with NA
  #file[[6]][abs(file[[6]])>1]<-NA
  ## convert raster data to data.table and assign Day of Year values
  ## EVI
  EVI.dt = as.data.table(as.data.frame(file[[5]], xy=T))
  EVI.dt = cbind(1:ncell(file[[5]]), EVI.dt)
  setnames(EVI.dt,c("Index","x","y", "EVI"))
  setkey(EVI.dt,Index,x,y)
  ## LSWI
  LSWI.dt = as.data.table(as.data.frame(file[[6]], xy=T))
  LSWI.dt = cbind(1:ncell(file[[6]]), LSWI.dt)
...
EVI_LSWI.dt <- rbind(EVI_LSWI1.dt,EVI_LSWI2.dt,EVI_LSWI3.dt,EVI_LSWI4.dt,EVI_LSWI5.dt,EVI_LSWI6.dt,
                     ...
                     EVI_LSWI43.dt,EVI_LSWI44.dt,EVI_LSWI45.dt,EVI_LSWI46.dt)
...
write.table(inter_evi_lswi,'C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/Borden_500m/adjusted_evi_lswi_interpolated_modis_snow_removed.csv',row.names = F,
            sep=',')
